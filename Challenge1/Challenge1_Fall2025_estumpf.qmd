...
3.  **Data Transformation: use necessary commands and codes and answer the following questions.**
    ...
    print("*** Question 1 Output ***")
    getUniqueCountForCol <- function(df, colNum) {
      count(distinct(df[colNum]))
    }
    print(sprintf(
      'Unique Sports: %s',
      getUniqueCountForCol(athleteEvents, 13)
    ))
    print(sprintf(
      'Unique Teams: %s',
      getUniqueCountForCol(athleteEvents, 7)
    ))
    print(sprintf(
      'Unique Names: %s',
      getUniqueCountForCol(athleteEvents, 2)
    ))
